// Pre-v1 implementation retained temporarily as migration reference.
import { z } from 'zod'
...
// Values copied into the signed dynamic-authorizer request must have one
// unambiguous representation in both the canonical message and HTTP headers.
// Restrict these config fields to visible ASCII (with optional interior
// spaces) so CR/LF injection, Unicode normalization differences, and header
// encoding differences cannot change what a receiver verifies.
const signedHeaderValueSchema = (field: string) =>
  z
    .string()
    .min(1)
    .max(256)
...
const envVarNameSchema = z
  ...
  .regex(
    /^[A-Za-z_][A-Za-z0-9_]*$/,
    'senders.dynamic.signingKeyEnv must be a valid environment variable name.',
  )

export const UNCONFIGURED_DYNAMIC_SENDER_IDENTITY = 'UNCONFIGURED'

const dynamicSenderIdentitySchema = z.string().refine(
  (identity) =>
    identity === UNCONFIGURED_DYNAMIC_SENDER_IDENTITY ||
    (isValidSuiAddress(identity) && identity === normalizeSuiAddress(identity)),
  'senders.dynamic.signingIdentity must be a canonical Sui address or UNCONFIGURED.',
)

const dynamicSenderConfigSchema = z
  .object({
    url: z.string().trim().min(1),
    audience: signedHeaderValueSchema('senders.dynamic.audience'),
    // Name of a Worker secret containing a Bech32 `suiprivkey...` value.
    // Required and explicit so each endpoint/trust domain can select a
    // dedicated signing identity. Raw keys are never accepted in policy JSON.
    signingKeyEnv: envVarNameSchema,
    // Public address expected from signingKeyEnv. UNCONFIGURED is a deliberate
    // fail-closed rollout sentinel and must only be used by disabled policies.
    signingIdentity: dynamicSenderIdentitySchema,
...
const policySchema = z
  .object({
    name: z
      .string()
      .min(1)
      .refine((name) => name.trim().length > 0, 'Policy name cannot be empty.'),
  ...
  .refine(
    (data) =>
      data.senders?.dynamic === undefined ||
      (data.name.length <= 256 &&
        /^[\x21-\x7e](?:[\x20-\x7e]*[\x21-\x7e])?$/.test(data.name)),
    'Policies with senders.dynamic require a visible-ASCII name with no leading or trailing whitespace.',
  )
  .refine(
    (data) =>
      data.senders?.dynamic?.signingIdentity !==
        UNCONFIGURED_DYNAMIC_SENDER_IDENTITY || data.enabled === false,
    'senders.dynamic.signingIdentity UNCONFIGURED is only valid on a disabled policy.',
  )
  .refine(
    (data) => {
...
const compilePolicy = (raw: z.infer<typeof policySchema>): CompiledPolicy => {
  // Preserve the historical normalization for non-dynamic policy names. A
  // dynamic policy is already required to arrive without surrounding space.
...
export const loadPolicies = (rawConfigs: unknown[]): CompiledPolicies => {
  ...
  for (const raw of parsed.data) {
    ...
    if (seenNames.has(normalizedName)) {
      throw new Error(`Duplicate sponsor policy name: ${normalizedName}`)
    }

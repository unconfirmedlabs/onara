...
Successful responses also include `policyDigest`, `policyVersion`, and
`engineVersion`, as described below. Both `/readyz` and `/status` send
`Cache-Control: no-store`, including their existing `503` failure responses.
...
```json
{
  ...
  "address": "0x...",
  "policyDigest": "sha256:<64 lowercase hex characters>",
  "policyVersion": 1,
  "engineVersion": "0.1.0",
  "balances": {
    "active": "1000000000",
...
`policyDigest` is `sha256:` followed by the lowercase SHA-256 hex digest of
the canonical UTF-8 encoding of the complete validated `{version, policies}`
configuration loaded by the runtime. Bun reads this envelope from
`ONARA_CONFIG_PATH`; Cloudflare bundles the same full envelope, including
when the deploy adapter generates an external configuration registry. The
runtime retains a frozen snapshot and computes its digest once at startup.

Canonicalization recursively sorts object keys by JavaScript UTF-16
lexicographic order (including integer-like keys: `"10"` precedes `"2"`),
preserves array order, and emits compact JSON with `JSON.stringify` encoding
for keys and primitive values. There is no Unicode normalization. Numbers
use JavaScript JSON number semantics: `1`, `1.0`, and `1e0` are identical,
and `-0` becomes `0`. Input is parsed as JSON, so whitespace, object key
order, and alternate JSON escapes do not affect the digest. As with
`JSON.parse`, repeated object keys resolve to the last value. Non-JSON
values, nonfinite numbers, sparse arrays, and cycles are rejected.

The digest is of the supplied JSON before policy compiler defaults or
normalization: omitted `enabled` and explicit `enabled: true` have different
digests, as do different address spellings and reordered arrays, even if
they enforce equivalent rules. `policyVersion` is the envelope's schema
version. `engineVersion` comes from `api/package.json`; bump that package
version whenever policy enforcement behavior changes, even when the schema
version remains the same.

With Bun installed, the API package provides:

```bash
onara policy-digest /path/to/config.json
```

From the repository root, use `bun run onara policy-digest /path/to/config.json`;
from `api/`, use `bun src/cli/index.ts policy-digest /path/to/config.json`.
The CLI uses the same validator and canonicalizer as the runtime. It needs
no server environment or RPC access, prints only the digest and a newline
to stdout, and exits nonzero with a stderr diagnostic for bad arguments,
unreadable files, invalid JSON, or invalid policy configurations.

The digest excludes environment variables (including `GAS_BUDGET_MAX` and
`DRY_RUN_ONLY`), secrets, deployment settings, and engine code. Compare the
engine version and relevant deployment settings separately. This is a
configuration fingerprint, not proof that a server actually enforces its
reported policy. The HTTP endpoints do not expose the configuration, but
the public unsalted digest permits guessing and checking candidate
configurations; do not treat it as a confidentiality mechanism.
...
The server loads a complete `{version: 1, policies: [...]}` configuration and
compiles its policies at startup. Bun reads the external configuration at
`ONARA_CONFIG_PATH`. Cloudflare uses the versioned configuration exported by
`policies/index.ts`, which imports in-tree JSON policies or is generated by
the deploy adapter from an external `config.json`.
...
For external deployments, add the policy to the `policies` array in your
`config.json`, retaining the top-level `version: 1` field.

For Cloudflare's in-tree configuration:

1. Create a JSON file in `policies/` (e.g. `policies/my-policy.json`)
2. Import it in `policies/index.ts` and add it to the versioned configuration's
   `policies` array:
...
const sponsorConfig = { version: 1, policies: [allowAll, myPolicy] }
...
src/
  core/
    ...
    policy-digest.ts Canonical JSON and validated policy fingerprints
    runtime.ts     Runtime configuration and shared dependencies
  cli/
    index.ts       Offline onara policy-digest command (Bun)
  http/
